...
class ValueIterationAgent(ValueEstimationAgent):
        ...
        for _ in range(self.iterations):
            ...
            for currState in self.mdp.getStates():
                if self.mdp.isTerminal(currState):
                    continue
                optimalAction = -float("inf")
                for action in self.mdp.getPossibleActions(currState):
                    optimalAction = max(optimalAction, self.getQValue(currState, action))
                values[currState] = optimalAction
            self.values = values
    ...
    def computeQValueFromValues(self, state, action):
        ...
        """
        "*** YOUR CODE HERE ***"
        transProbList = self.mdp.getTransitionStatesAndProbs(state, action)
        qVal = 0
        for nextState, prob in transProbList:
