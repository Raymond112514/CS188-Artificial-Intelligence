...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict[PAC] = list(pos)
    variableDomainsDict[GHOST0] = list(pos)
    variableDomainsDict[GHOST1] = list(pos)
    variableDomainsDict[OBS0] = list(obs)
    variableDomainsDict[OBS1] = list(obs)
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        # Step 1. Get the current list of evidence
        currentFactorsList = bayesNet.getAllCPTsWithEvidence(evidenceDict)

        # Step 2. Update the current list of evidence
        for factor in eliminationOrder:
            currentFactorsList, joinedFactor = joinFactorsByVariable(currentFactorsList, factor)
            if len(joinedFactor.unconditionedVariables()) > 1:
                eliminatedFactor = eliminate(joinedFactor, factor)
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        probability = list(self.values())
        rand =  random.choices(variables, weights=probability)
        return rand[0]
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        # Step 1: for each legal position update belief distribution using forward algorithm
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        transitionProb = util.Counter()

        # Step 1. Retrieve the transition probability for all positions
        for pos in self.allPositions:
            transitionProb[pos] = self.getPositionDistribution(gameState, pos)

        # Step 2. Update belief distribution using forward algorithm
        for pos in self.allPositions:
            prob = 0
            for prevPos in self.beliefs:
                prob += transitionProb[prevPos][pos] * self.beliefs[prevPos]
            updatedBeliefs[pos] = prob

        # Step 3. Normalize the updated beliefs
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        numAvailablePos = len(self.legalPositions)
        particle = self.legalPositions * (self.numParticles // numAvailablePos)
        particle += random.sample(self.legalPositions, self.numParticles % numAvailablePos)
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        # If all particle recieves 0 weights, initialize randomly, else randomly sample
