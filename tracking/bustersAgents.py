...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        ghostPos = [distribution.argMax() for distribution in livingGhostPositionDistributions]

        # Step 2. Compute the closest ghost
        def distToPacman(pos):
            return self.distancer.getDistance(pacmanPosition, pos)
        closestGhost = min(ghostPos, key=distToPacman)

        # Step 3. Determine the optimal action
        def distToGhostAfterAction(action):
            successor = Actions.getSuccessor(pacmanPosition, action)
            return self.distancer.getDistance(successor, closestGhost)
        optimalAction = min(legal, key=distToGhostAfterAction)
